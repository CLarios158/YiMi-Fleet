import { Container, Content, Text, Input, Button, Grid, View, Col, Row, H3 } from 'native-base';
...
class CodigoV extends Component {
  ...
  verificateCode = () => {
    ...
    if (this.state.isConnected != false) {
      axios.post('http://35.203.57.92:3000/validar_codigo', { telefono: this.state.telefono, codigo: codigoV })
        .then(response => {
          if (response.data.respuesta == 0) {
            ...
            axios.put('http://35.203.57.92:3000/modificar_usuario', {
              ...
              .then(response => {
                ...
                Alert.alert(
                  'Aviso',
                  'Se ha actualizo tu número de teléfono con éxito!',
                  [
                    { text: 'OK', onPress: () => this.props.navigation.navigate('Configuracion', { id_usuario: this.state.id_usuario }) },
                  ]
                );
              })
      ...
      <Container>
        <Content>
          <Grid>
            ...
            <Row style={{ marginTop: 20, marginBottom: 10, marginLeft: 10 }}><H3>Código de verificación</H3></Row>
            ...
            <Row style={{ marginTop: 40 }}>
              <Col style={{ display: 'flex', alignContent: 'center', alignItems: 'center', marginLeft: 85, marginRight: 85 }}>
                <Button onPress={this.verificateCode} block style={{ backgroundColor: '#ff8834' }}><Text>Siguiente</Text></Button>
              </Col>
            </Row>
          </Grid>
        </Content>
