...
class Login extends Component {
  ...
  GetSession = async () => {
    try {
      ...
      if (campo != null && passwd != null) {
        ...
        axios.post('http://35.203.57.92:3000/validar_inicio_sesion', { id_rol: 1, campo: this.state.campo, pass: this.state.contrasena })
          .then(response => {
  ...
  SignIn = () => {
    if (this.state.isConnected != false) {
      axios.post('http://35.203.57.92:3000/validar_cuenta', { campo: this.state.campo })
        .then(response => {
          response.data.data.forEach(element => {
            if (element["respuesta"] == 0) {
              this.setState(() => ({ nameError: "La cuenta no existe" }));
            } else {
              axios.post('http://35.203.57.92:3000/validar_inicio_sesion', { id_rol: 2, campo: this.state.campo, pass: this.state.contrasena })
                .then(response => {
                  response.data.data.forEach(element => {
                    if (element["respuesta"] == 0) {
  ...
  goForgetPasswd = () => {
    if (this.state.isConnected != false) {
      if (this.state.campo != '') {
        axios.post('http://35.203.57.92:3000/validar_cuenta', { campo: this.state.campo })
          .then(response => {
            ...
            if (this.state.respuesta == 0) {
              ...
              this.setState({ campo: '', contrasena: '', nameError: '' });
              this.props.navigation.navigate('RestablecerContrasena', { id_usuario: this.state.id_usuario })
            }
          })
          .catch(function (error) {
            // handle error
        ...
        Alert.alert(
          'Aviso',
          'Ingrese su número de teléfono o correo eléctronico',
          [
            { text: 'OK' }
          ]
        );
