...
import { NetInfo, Dimensions, KeyboardAvoidingView, AsyncStorage, ActivityIndicator} from 'react-native';
import { withNavigation } from 'react-navigation';
import axios from 'axios';
const { width } = Dimensions.get('window');

class DocumentoSC extends Component {
    constructor(props) {
        super(props);
        this.state = {
            ...
            estado_licencia: null, archivo_licencia:'',
            estado_foto: null, archivo_perfil: '',
            estado_carta: null, archivo_carta:'',
            estado_publico: null, estado_rfc: null,
            estado_privado: null, estado_csd: null,
            estado_pruebat: null, archivo_prueba:'',
            estado_comprobante: null, archivo_comprobante:'',
            fisica: true, moral: false, isConnected:true, loading: false,
            archivo_publico: '', archivo_privado: '', tipo_rfc: 1,
            rfc: '', csd:''
        };
    }
    ...
    IdentificacionOficial = () => {
        this.props.navigation.navigate('IdentificacionOficialSC', {id_usuario: this.state.id_usuario, estado: this.state.estado_identificacion, foto: this.state.archivo_identificacion});
    }

    LicenciaConducir = () => {
        this.props.navigation.navigate('LicenciaConducirSC', {id_usuario: this.state.id_usuario, estado: this.state.estado_licencia, foto: this.state.archivo_licencia});
    }

    CartaAntecedentes = () => {
        this.props.navigation.navigate('CartaAntecedentesPenalesSC', {id_usuario: this.state.id_usuario, estado: this.state.estado_carta, foto: this.state.archivo_carta});
    }

    ComprobanteDomicilio = () => {
        this.props.navigation.navigate('ComprobanteDomicilioSC', {id_usuario: this.state.id_usuario, estado: this.state.estado_comprobante, foto: this.state.archivo_comprobante});
    }

    PruebaToxicologica = () => {
        this.props.navigation.navigate('PruebaToxicologicaSC', {id_usuario: this.state.id_usuario, estado: this.state.estado_pruebat, foto: this.state.archivo_prueba});
    }

    FotoPerfil = () => {
        this.props.navigation.navigate('FotoPerfilSC', {id_usuario: this.state.id_usuario, estado: this.state.estado_foto, foto: this.state.archivo_perfil});
    }
    
    ArchivoPublico = () => {
        this.props.navigation.navigate('ArchivoPublicoSC', {id_usuario: this.state.id_usuario, estado: this.state.estado_publico, name: this.state.archivo_publico});
    }

    ArchivoPrivado = () => {
        this.props.navigation.navigate('ArchivoPrivadoSC', {id_usuario: this.state.id_usuario, estado: this.state.estado_privado, name: this.state.namePrivado});
    }
    ...
    CargarDocumentoRFC = () => {
        ...
        if(this.state.isConnected == true){
            ...
            axios.post('http://35.203.42.33:3000/registrar_documento',{archivo: this.state.rfc, id_usuario: this.state.id_usuario, id_cat_documento: 4, tipo_rfc: this.state.tipo_rfc})
            .then(response => {
                // handle success
    ...
    CargarDocumentoCSD = () => {
        ...
        if(this.state.isConnected == true){
            ...
            axios.post('http://35.203.42.33:3000/registrar_documento',{archivo: this.state.csd, id_usuario: this.state.id_usuario, id_cat_documento: 7, tipo_rfc: null})
            .then(response => {
                // handle success
    ...
    onValidateInput = () => {
        ...
        if(this.state.estado_identificacion == 0  
            ...
            this.SaveStatus();
            this.props.navigation.navigate('CuentaBancariaRegistro', {id_usuario: this.state.id_usuario, rol: 2, telefono: this.state.telefono, pass: this.state.pass})
        }else{
                ...
                <Container>
                    <Content>
                        <Grid>
                            ...
                            <Row><Col><Button onPress={this.onValidateInput} block style = {{marginLeft: 20, marginRight: 20, marginTop:20, marginBottom:10, backgroundColor:'#ff8834'}}><Text>Siguiente</Text></Button></Col></Row>
                        </Grid> 
                    </Content>
                </Container>
            );
...
export default withNavigation(DocumentoSC);