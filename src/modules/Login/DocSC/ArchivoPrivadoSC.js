        ...
        if (result.type == 'cancel') {
            ...
            if(!allowedExtensions.exec(this.state.name)){
                ...
                if(this.state.DocumentoPrivado != "" && allowedExtensions.exec(this.state.name)){
                    this.props.navigation.navigate('CargarArchivoPrivadoSC',{archivo: this.state.DocumentoPrivado, name: this.state.name, id_usuario: this.state.id_usuario, id_cat_documento:6, estado: this.state.estado});
                }
            }
        } 
    }
