...
class Correo extends Component {
    ...
    updateCorreo = () => {
        if (this.state.isConnected == true) {
            axios.post('http://35.203.57.92:3000/mail', { correo: this.state.correo2 })
                .then(response => {
                    axios.put('http://35.203.57.92:3000/modificar_usuario', {
                        id_usuario: this.state.id_usuario, nombre: this.state.nombre, apellido: this.state.apellido, correo: this.state.correo2,
            ...
            <Container>
                <Content>
                    <Grid>
                        ...
                        <Row><Col style={{ marginLeft: 20, marginTop: 20 }}><H3>Ingresa su nuevo correo para guardar</H3></Col></Row>
                        <Row>
