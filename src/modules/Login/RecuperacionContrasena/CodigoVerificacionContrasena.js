    ...
    if(this.state.isConnected == true){
      ...
      .then(response => {
        if(response.data.respuesta == 0){
          ...
          this.props.navigation.navigate('RecuperacionContraseña', {id_usuario: this.state.id_usuario, isConnected: this.state.isConnected})
        }
      })
      ...
      <Container>
        <Content>
          <Grid>
            ...
            <Row style = {{marginBottom: 40, marginLeft:20}}><Text style={{fontSize:12}}>El código de verificación se ha enviado al: {this.state.telefono}</Text></Row>
            ...
            <Row style={{marginTop:10}}>
              <Col><Button disable={true} transparent onPress={this.enviarCodigo}><Text style={{color:'black'}}>No recibí el código ></Text></Button></Col>
            ...
            <Row style={{marginTop:40}}>
              <Col style={{display:'flex', alignContent:'center', alignItems:'center', marginLeft:85, marginRight:85}}>
                <Button onPress={this.verificarCodigo} block style={{backgroundColor:'#ff8834'}}><Text>Validar</Text></Button>
              </Col>
            </Row>
          </Grid>
        </Content>
