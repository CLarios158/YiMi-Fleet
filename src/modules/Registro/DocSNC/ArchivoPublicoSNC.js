        ...
        if (result.type == 'cancel') {
            ...
            if(!allowedExtensions.exec(this.state.name)){
                ...
                if(this.state.DocumentoPublico != "" && allowedExtensions.exec(this.state.name)){
                    this.props.navigation.navigate('CargarArchivoPublicoSNC',{archivo: this.state.DocumentoPublico, name: this.state.name, id_usuario: this.state.id_usuario, id_cat_documento:5, estado: this.state.estado});            
                }
            }
        }
                    ...
                    <Grid>
                        ...
                        <Row>
                            {this.state.estado == 0
                                ...
                                :<Col style={{display:'flex', alignItems:'center',alignContent:'center', marginTop:10, marginLeft:20, marginRight:20}}>
                                    <Text style = {{textAlign:'center', fontWeight:'bold'}}>Archivo Cargado: {this.state.nameCargado}</Text>
                                 </Col>
                            }
                        </Row>
                        <Row>
