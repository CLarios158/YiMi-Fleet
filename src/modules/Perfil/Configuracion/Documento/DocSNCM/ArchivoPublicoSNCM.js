        ...
        if (result.type == 'cancel') {
            ...
            if(!allowedExtensions.exec(this.state.name)){
                ...
                if(this.state.DocumentoPublico != "" && allowedExtensions.exec(this.state.name)){
                    this.props.navigation.navigate('CargarArchivoPublicoSNCM',{archivo: this.state.DocumentoPublico, name: this.state.name, id_usuario: this.state.id_usuario, id_cat_documento:5, estado: this.state.estado});            
                }
            }
        } 
    }
